...
public class SipResponseEncoderTest extends AbstractSipTest {
	...
	public void testMessageBadRequest() throws Exception {
		testMessageBadRequest(new SipResponseImpl(SipVersion.SIP_2_0, SipResponseStatus.BAD_REQUEST));
	}
	protected void testMessageBadRequest(SipMessage msg) {
		...
		Assert.assertEquals("SIP/2.0 400 Bad Request", buffer.toString(SipMessageEncoder.charSet).trim());
	}
	...
	protected void testHeaders(SipResponse msg) throws Exception {
		...
		ChannelBuffer buffer = embedder.poll();
		String content = prepare(getFileContent("bob_ack_invite.txt", SipMessageEncoder.charSet));
		String generated = prepare((buffer.toString(SipMessageEncoder.charSet)));		
